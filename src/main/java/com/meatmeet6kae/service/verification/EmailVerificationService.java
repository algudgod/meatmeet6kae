...
public class EmailVerificationService {
    ...
    public void sendVerification(String email, String token) throws MessagingException {
        ...
        String subject = "Please Complete Your Email Verification";
        String confirmationUrl = "http://localhost:8080/email/confirm?token=" + token;
        String message = "To complete your email verification, please click the following link: \n" + confirmationUrl;
        ...
        helper.setFrom("92mi_hyung@naver.com");
        helper.setSubject(subject);
        helper.setText(message, true); // true는 HTML을 허용하는 것이며 메세지 본문에 HTML 태그를 포함할 수 있다.

        logger.debug("Attempting to send email to: {}", email); // 로그: 이메일 전송 시도
        javaMailSender.send(mimeMessage); // 완성된 이메일을 전송
        logger.debug("Email sent successfully to: {}", email); // 로그: 이메일 전송 성공
    }
